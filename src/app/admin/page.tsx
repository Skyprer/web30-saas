...
import { Store, Phone, MapPin, CheckCircle, Type, Package, Image as ImageIcon } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (error) alert("Gagal menyimpan: " + error.message);
      ...
      setNomorWa(formattedWa); 
      setSuccessMsg("Pengaturan toko berhasil diperbarui!");
      setTimeout(() => setSuccessMsg(''), 3000);
    }
  };

  if (isLoading) return <div className="min-h-screen flex items-center justify-center font-bold text-slate-500">Memuat Dashboard...</div>;
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 animate-fade-in pb-24">
      ...
      {successMsg && (
        <div className="mb-6 bg-green-50 border-2 border-green-200 text-green-700 p-4 rounded-xl flex items-center gap-3 shadow-sm">
      ...
      <form onSubmit={handleSave} className="bg-white rounded-2xl shadow-sm border-2 border-slate-200 p-6 md:p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-black text-slate-900 mb-2 flex items-center gap-2"><Phone className="w-4 h-4 text-blue-600"/> Nomor WhatsApp</label>
              ...
              <p className="text-xs font-bold text-slate-500 mt-2">Pesanan akan otomatis masuk ke nomor ini.</p>
            </div>
          </div>

          <div className="space-y-6">
            <div>
              <label className="block text-sm font-black text-slate-900 mb-2 flex items-center gap-2"><Type className="w-4 h-4 text-blue-600"/> Tagline / Slogan Banner</label>
